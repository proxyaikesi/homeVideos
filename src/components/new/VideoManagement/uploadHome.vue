  ...
  <form @submit.prevent="handleSubmit">
    <div class="info_box">
      ...
      <div class="file_tl">
        ...
        <div
          class="row obtain"
          @click.stop="btn_b($event)"
          v-bind:class="{ iptline: shows == 2 }"
        >
      ...
      <div class="describe">
        ...
        <textarea
          ...
          @click.stop="btn_c($event)"
          v-bind:class="{ iptline: shows == 3 }"
        ></textarea>
      </div>
      <div class="classify">
        <p>分类</p>
      ...
      <div class="location">
        ...
        <div
          class="row obtain"
          @click.stop="btn_f($event)"
          v-bind:class="{ iptline: shows == 6 }"
        >
...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
      axios
        .post("http://localhost:3000/videoInfo", {
          params: {
            info: this.formdata,
    ...
    async btn() {
      ...
      if (this.$store.state.projectTitleId[0] == undefined) {
        ...
        while (uploadedSize < size) {
          const fileChunk = this.$store.state.projectTitleId[0].id.slice(
            uploadedSize,
            uploadedSize + this.CHUNK_SIZE
          );
          ...
          try {
            uploadedResult = await axios.post(
              "http://localhost:3000/uploadhome",
              fileFormData
            );
          } catch (error) {
            console.log(error);
...
.boder_box:hover {
  box-shadow: inset 0 0 0 2px hsla(240, 8%, 88%, 0.4);
}
